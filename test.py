...
class spider(object):
    def __init__(self):
        ...
        self.page_limit = 10
    def init(self):
        if not os.path.exists("./tempData2.csv"):
            with open("./tempData2.csv", 'w', newline='') as writer_f:
        ...
        if not os.path.exists('./spidersPage2.txt'):
            with open("./spidersPage2.txt", 'w', encoding='utf-8') as writer_f:
                writer_f.write('0\n')

    def get_spiderPage(self):
        with open('./spidersPage2.txt', 'r') as f:
            return f.readlines()[-1].strip()

    def set_spiderPage(self, newPage):
        with open('./spidersPage2.txt', 'a') as f:  # 'a'追加模式，在文件末尾添加数据，不覆盖原有数据
            f.write(str(newPage)+'\n')  # 添加内容为：newPage + '\n'
    ...
    def spidermain(self):
        ...
        for index, movieData in enumerate(respJson):
            ...
            for i in comments_list:
                ...
                star = re.search(r'\d+', i.xpath('.//h3/span[2]/span[2]/@class')[0]).group()
    ...
    def save_to_csv(self, resultList):
        with open('./tempData2.csv', 'a', newline='', encoding='utf-8') as csvfile:
